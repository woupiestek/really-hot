...
Let $n>1$ and let $f\of X\to\horn_k[n]$ be an arbitrary fibration. This section shows to construct a fibration $D(f)\of D(X)\to \simplex[n]$ such that $f$ is a pullback of $D(f)$ along $f$.

%TODO: Explain the construction informally.

\begin{definition} For $\xi\of [m]\to[n]$ define $l(\xi)\of L(\xi)\to\horn_k[n]$ as follows. 
\begin{align*}
 L(\xi) &= \set{\phi:[l]\to [m+\norm\xi]\middle| K_0(\xi)\circ\phi \in \base(\horn_k[n]) }&
 \dim(\phi) &= \dom(\phi)\\
 \phi\cdot\chi &= \phi\circ\chi&
 l(\xi)(\phi) &= K_0(\xi)\circ\phi
\end{align*}

For any $f\of X\to\horn_k[n]$ define $Df\of DX\to\simplex[n]$ as follows.
\begin{align*}
D(X) &= \set{\tuplet{\xi,x:l(\xi)\to f}\middle|\xi\in\base(\simplex[n])}&
\dim\tuplet{\xi,x} &= \dim(\xi)\\
\tuplet{\xi,x}\cdot\phi &= \tuplet{\xi\circ\phi,x\circ\simplex(\phi)}&
D(f)\tuplet{\xi,x} &= \xi
\end{align*}

\end{definition}

\begin{lemma} The functors $(\ambient\s/\horn_k[n])(l(\xi),f)$ and $(\ambient\s/\simplex[n])(\simplex(\xi),D(f))$ are naturally isomorphic. \end{lemma}
