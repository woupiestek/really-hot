...
\hide{%here
Consider we are mixing objects of $\ambient$ and $\ambient\s$, so be careful.
...
The transfinite compositions relies on the projectives of $\ambient\s$. Like $\ambient$, $\ambient\s$ has enough projectives. \hide{
From $X_i\to X_{i+1}$ to $Y^{X_{i+1}}\to Y^{X_i}$. A projective cover $\to Y^X$ induces a cochain morphism $\nno\ri Y\to f$ by induction. This must factor through the transfinite composition.
}
...
If an $s\ri$-algebra is a regular epimorphism, then so are its transfinite compositions.%Better this way!?
...
\begin{proof} Let $Z$ be the following object of $\ambient$. For each $n\of\nno$, $Z[n]$ consist of quadruples $\tuplet{a,b,c,d}$ where 
