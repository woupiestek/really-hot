...
\newcommand\product[1]{\Pi #1 \mapsto}
\newcommand\coproduct[1]{\Sigma #1 \mapsto}
\newcommand\function[1]{\lambda #1 \mapsto}
...
\hide{
The weakest points so far are the generalizations over families of horn and cycle inclusions. It simply is not always the case that the natural number object is the coproduct of countable copies of $1$, so we rely on the argument that the reasoning is constructive, and can therefore be internalized. Let's try to do better.
}
