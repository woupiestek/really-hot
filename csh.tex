...
Idea: define Kanfibrations, define cofibrations, demonstrate the lifting properties. Move on happily.
...
\hide{ What about acyclic cofibrations?

We can introduce fibration without talking about acyclic cofibrations. We only need acyclic cofibrations between fibrant objects, and those are a kind of deformation retract--the first once mentioned in this paper where the homotopy commutes with the projection.

There is an equivalence, so we can use the deformation retracts to define acyclic cofibrations. Great!
}
