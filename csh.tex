...
\hide{
The construction of $D$ is not a problem. The problem is its left adjoint.

Combining $h\ri$ and $K$ is a bad idea. May work in a Beck Chevalley context.

Decidable sieves are enough, we don't need compacts.
}
...
\begin{proof}
The functor $D$ is a composition $K^*\circ h_*$ where $h_*\of \ambient\s/\horn_k[n]\to\ambient\s/\simplex[n]$ is the right adjoint of $h\ri$. The functor $K^*$ is the left Kan extension of the embedding $\simplex\of\simCat \to \ambient\s$ of the simplex category along a functor $K\of\simCat/[n]\to \ambient\s/\horn_k[n]$, see \ref{the functor K}. %TODO--how do we use K?

The category $\ambient\s$ may have too few colimits to give $D$ a left adjoint. There is however a left Kan extension $K_!$ of $K$ to the subcategory $\compact/\simplex[n]$ where $\compact$ consists of finite colimits of simplices. %TODO
The functor $h\ri\circ K_!$ preserves acyclic cofibrations and therefore $D$ preserves fibrations. %TODO
\end{proof}
