...
\begin{lemma} For $n>1$ there is a functor $D\of \ambient\s/\horn_k[n]\to\ambient\s/\simplex[n]$ which preserves fibrations and for which $h\ri\circ D$ is naturally equivalent to the identity functor. \end{lemma}

\newcommand\compact{\mathfrak{K}}
\begin{proof}
Let $\simplex\of\simCat \to \ambient\s$ be the embedding of the simplex category in the category of simplicial sets. There is a functor $K\of\simCat/[n]\to \ambient\s/\horn_k[n]$ that sends morphisms $\xi\of [m]\to[n]$ to weak equivalences $K\xi\of K[m]\to \horn_k[n]$, see \ref{the functor K}.%TODO
A simplicial object is \emph{compact} if it is covered by a finite coproduct of simplices--for example all $\horn_k[n]$ are compact. Let $\compact$ be the full subcategory of $\ambient\s$ whose objects are compact. There is a \emph{right Kan extension} of $K$ along the inclusion $\compact/\simplex[n]\to\ambient\s/\horn^k[n]$ and this extension preserves acyclic cofibrations. %TODO
There is a \emph{left Kan extension} $D$ of the inclusion $\simCat/[n]\to\ambient\s/[n]$ along $K$ which has all the desired properties. %TODO
\end{proof}


\hide{ 
We probably need more properties to prove that the left extension is inverse to pulling back along $h$
The morphisms aren't properly 'acyclic cofibrations' since their domains and codomains aren't fibrant.
The morphisms $K(\xi)$ are not cofibrations. They probably are acyclic fibrations.
}
...
\begin{lemma} The functor $K$ has a left Kan extension $\tilde K$ along the inclusion $I\of \simCat/[n]\to\compact/\simplex[n]$. \label{left extension} \end{lemma}

\begin{proof} The members of $\compact$ are finite colimits of simplexes. The category $\ambient$ has all finite colimits and therefore $\ambient\s/\horn_k[n]$ does too.\hide{do people know this?} The functor $\tilde K$ is simply equals $K$ for simplices and is defined to preserve finite colimits otherwise.
For every functor $F\of\compact/\simplex[n]\to\ambient\s/\horn_k[n]$ and every natural transformation $\phi\of K\to FI$ there is a unique natural transformation $\tilde\phi\of \tilde K\to F$ such that $\tilde\phi I$ is $\phi$ composed with the isomorphism $\tilde KI\to K$. This follows from the universal properties of finite colimits. \hide{constructiveness: there must be a specified diagram to witness compactness}
\end{proof}%TODO: should this not be a right extension!?

%TODO acyclic cofibrations preservation lemma

\begin{lemma} The inclusion $\simplex\of\simplex/[n]\to\ambient\s/\simplex[n]$ has a right Kan extension $D$ along $K$. \label{right extension} \end{lemma}
...
The natural transformation $\epsilon\of \simplex \to DK$ sends $\alpha\of \xi\circ \alpha \to \xi$ to $\tuplet{\xi,K_1\alpha}$. %TODO
%It looks like another left Kan extension. Is the other right after all!?
\end{proof}
