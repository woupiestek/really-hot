...
Here is a rough sketch to explain the intuition behind the construction.
For each $\xi\of [m]\to[n]$ the simplexes in the fibre of $Df$ over $\xi$ is a combination of a morphism $h\ri(\simplex(\xi))\to f$ together with a homotopy with a simplex in the fibre over the top edge $k$ of $\horn_k[n]$. The extra simplex records factorizations of morphisms $\simplex[k]\to \simplex[n]$ through $\xi$ that don't factor through $h$ and are otherwise lost in the pullback. With the homotopy the fibration $Df$ can lift horns by letting $f$ lift a homotopic morphisms, for which $f$ has the right lifting property because it is a fibration.
