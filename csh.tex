...
%TODO: we may need a type of transfinite composition to deal with unbounded families of finite limits. Modest sets have these becaue they are a right class in some factorization.
...
The following functor $K$ sends each $\simplex[n]$-indexed family of simplices to the family of simplices it will be a face of.
...
\begin{lemma} Let $Df = K\ri h_* f\of K\ri h_*X\to\simplex[n]$. Each $f\of X\to\horn_k[n]$ is a pullback of $Df$ along $H(n,k)$. \label{f is pullback}\end{lemma}
...
$\depsum K H(m,l)$ has a counterpart in $\ambient\s/\simplex[n]$ which has a pullback $h\ri \depsum K H(m,l)$ in $\ambient\s/\horn_k[n]$. The following examples of acyclic cofibrations helps to prove that this is an acyclic cofibration.
...
\begin{lemma} In $\ambient\s/\horn_k[n]$, $h\ri \depsum K H(m,l)$ is a subobject of $H(n,l)\ri\simplex[K\xi]$ . The inclusion of their domains in $\ambient\s$, $h\ri \depsum K H(m,l)$ is an acyclic cofibration. \label{acyclic cofibrancy} 
\[\xymatrix{
...
The map $H(n,l)\ri \depsum K H(m,l)\to L$ is an acyclic cofibration because it is a pushout of the coproduct of these simpler face inclusions. If $\xi(l)=k$, then $L=h\ri \simplex[K\xi]$ and the proof is done. Otherwise the face $\face^{K\xi}(\U(\xi(l)))$ is left.
