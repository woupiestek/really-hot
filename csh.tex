...
\begin{proof} Let $X$ be a simplicial object. There is a projective cover $c\of Y\to\base X$ in $\ambient$. Let $\base Z$ be the object of pairs $\tuplet{x,y}\of Y\times\Ar(\simCat)$ where $y$ is an epimorphism $[n]\to [\dim c(x)]$ for some $n\of\nno$. Let $\dim\tuplet{x,y} = \dom y$. 

The restriction map is more complicated to define. Every morphism $\chi$ of $\simCat$ factors as a monomorphism $m(\chi)$ following an epimorphism $e(\chi)$. Let $\tuplet{x,y}\cdot\phi = \tuplet{ x\cdot m(y\circ \phi), e)$ where $x\cdot \mu$ for monomorphisms $\mu$ of $\simCat$ is defined as follows.

We rely on dependent choice to define the operator for monomorphisms $\mu\of[i] \to [i+1]$. In each step the constraint is that $c(x\cdot \mu) = c(x)\cdot\mu$. For $i=0$ choose $x\cdot \mu$ freely within that constraint. If $\cdot$ is defined for $i$ and $\mu\of[i+1]\to [i+2]$, then choose $x\cdot \mu$ in such a way that $x\cdot \mu\circ \nu$ always agrees with earlier choices. %unclear

\hide{Once again, we need a stepwise approach to ensure commutativity

Look at the subcategory of monomorphisms, and gradually extend the functors. 

First all the [0]\to [n]. These as chosen freely.
[1]\to [n] Have to agree with former choices for the endpoints. 
[2]\to [n] Have to agree with former choices for the paths. Is this always possible?
etc.
}



% doesn't it fail in the lowest dimension? 1+1\to \simplex[1]!?

%restricting to cofibration object becomes more attractive all the time.

The faces of $Z$ are the pairs $\tuplet{x,y}$ where $x$ is a monomorphism.
...
\begin{proof} Let $f\of X\to Y$ be any morphism of $\ambient/s$. Let $Y'$ be a projective cover of $\base f$. 

For each $n\of\nno$, $Z[n]$ consist of quadruples $\tuplet{a,c,d}$ where 
