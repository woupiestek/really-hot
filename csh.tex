...
\begin{abstract} The paper show how to develop simplicial homotopy inside the category of assemblies, where the small object argument and minimal fibrations are unavailable. This way simplicial assemblies are a suitable model for homotopy type theory.\end{abstract}

\maketitle

\section{Introduction}
This paper generalizes the classical model structure on simplicial sets to (internal) simplicial objects of \emph{extensive locally Cartesian closed categories}. Many of the traditional arguments rely on the decidability of monomorphisms, which is no longer valid here. The solution is to wrok with internalized simplicial objects, to limit the class of cofibrations and only develop the model structure on relatively well behaved fibrant objects (theorem \ref{model category}).
...
\item There is an internal full subcategory of small assemblies that is extensive and locally Cartesian closed and that has a terminal object.
\end{enumerate}

\newcommand\icat{\mathfrak}
This last point requires an explanation. 

The category of small assemblies is both a full subcategory $\icat S$ of $\cat A$ and an internal category $\cat S$ of $\cat A$. They are connected by a class of \emph{small morphisms} $S$ in $\icat S$ which is closed under pullbacks. The category $\icat S$ is the full subcategory of objects $X$ for which the unique morphism $\bang\of X\to 1$ is in $\icat S$.

The class $S$ has a \emph{generic} morphism $\gen(S)$. `Generic' means that every small morphism $f\of X\to Y$ is the pullback along some \emph{characteristic} morphism $\chi(f)\of Y\to \cod(\gen(S))$--here $\cod(\gen(S))$ is the codomain of $\gen(S)$. The internal category $\cat S$ is the internal category of fibres of $\gen(S)$. This category has $\cod(\gen(S))$ as object of objects $\Ob(\cat S)$. The object of arrows $\Ar(\cat S)$ is the domain of the exponential of the morphisms $\gen(S)\times \id_{\Ob(\cat S)}$ and $\gen(S)\times \id_{\Ob(\cat S)}$ which are objects of the Cartesian closed slice category $\icat A/\Ob(\cat S)\times \Ob(\cat S)$. 
...
\hide{does this require a separate explanation?}
