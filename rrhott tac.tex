...
%begin clarification
This time, $M$ is internal to $\cat S$ too. Since internal categories like $\cat S$ have no set of morphisms of which $M$ can be a subset, $M$ takes a different form.

\mathssbxdef{Iso}

\begin{definition}
A discrete isofibration is a functor $F\of\cat A\to \cat B$ which is fully faithful and which has a lifting property for isomorphisms. If $A$ is an object of $\cat A$ and if $f\of B\to FA$ is an isomorphism of $\cat B$ then there is an isomorphism $f'\of B'\to A$ such that $Ff'=f$. The internal version for locally Cartesian closed categories has a lifting operator, so $f'=l(X,f)$ for some element $l$ of.
\[ \Pi X\of \Ob(\cat A).\Pi f\of \Iso(\cat B)/FX. \Iso(\cat A)/X \]
Here $\Iso(\cat X)/Y$ is the object of isomorphisms of $\cat X$ whose codomains are $Y$.
\end{definition}
...
A morphism $\pair fs\to \pair gt$ is a commutative square $f\to g$. The pullback squares are ignored, so the projection $\pi_0$ becomes a fully faithful functor $M\of\cat M\to \Func([1],\cat S)$. Because isomorphisms of morphisms are pullbacks squares, the lifting operator that witnesses that $M$ is a discrete isofibration simply composes the isomorphisms and the pullback squares: $(\pair fs,g)\mapsto \pair {\dom g}{s\circ g}$.

There is a related discrete isofibration $M\s\of \cat M\s\to \Func([1],\cat S\s)$ which represent the class of morphisms $f\of X\to Y$ such that $f[i]$ are members of $M$ for all $i\of \nno$. The category $\cat M\s$ is simply the category of functors $\Func(\simCat\dual,\cat M)$ and $M\s$ acts by composition with $M$.
...
For each $\phi\of \simCat([m],[n])$, $\cod(\gen(M\s\f))(\phi)(x,y) = (\cod(\igen(M\s))(\phi)(x),y')$, where $y'$ is $y$ transported along $\simplex(\phi)$.
\end{definition}
