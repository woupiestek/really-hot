...
%here
\hide{revisit the class of weak equivalences here?

It seems that if there is a factorization into acyclics, any fibration-cofibration factorization that has one acyclic has two.

While it must be possible to use a specific factorisation everywhere, it can't be simple.
}
...
\[ \Pi X\of \Ob(\cat A).\Pi f\of \Ob(\Core(\cat B)/FX). \Ob(\Core(\cat A)/X) \]
\label{discrete isofibrations}
\end{definition}
