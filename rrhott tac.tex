...
\hide{ Onderverdelen: maak van elke stelling een apart artikel. Misschien betekent dat veel herhaling. Misschien betekent dat snellere resultaten.

Kleinere projecten en snellere resultaten.


Ik heb iets over modelstructuren
En iets over een univalent universum
Twee artikeltjes.

Misschien moet ik het beperken tot een belangrijk lemma: constructive descent. Dan ben ik daar alvast van af.


* fibrations' constructive descent

A fibration $f:X\to Y$ has certain interesting properties that imply that its fibres are closed under finite limits. We just need something for the diagonal and for subobjects. 

It follows that $Y$ is a Kan complex. We prove this constructively and in fact with less than the usual amount of logic.

The statement should hold for fibrations in internal categories inside lcccs with (stable) nno's. By externalizing and using generalized elements, we can lift this to just a normal category with a certain model structure on it. I.e. use $f:I\to \Ob(\Func(\simCat,\cat S))$ to denote some generic fibration. I have to show that this works though. Stack semantics might help somewhere.

Maybe it is better to proof the concrete example (the simplicial objects of an extensive lccc) and than generalize it to internal extensive lcccs of extensive lcccs with nnos. The question is then whether we should split these into two smaller papers. Let's just see how this works out.

The paper still requires setting up the locally decidable fibrations. Maybe we don't need the model structure bit yet.
}
...
\begin{definition}
Since $\cat S$ may have too small products, the property of being $a$-anodyne for an $I$-indexed family of morphisms $a$ is represented by a functor valued in the following Heyting bialgebra.
...
For a definition of model categories see [\cite{Hovey99, GJSHT}]. The simplicial homotopy of Kan complexes is valid in the internal Heyting bialgebra $\cat S$ in the following sense.
...
\hide{We are gonna go without the discrete cofibrations}
\section{Fibrant Universe}
This section adds a new class of \emph{modest morphisms} $M\subseteq S$ with its own generic morphism $\gen(M)$ and full internal subcategory $\cof(\gen(M))$. The class has the following properties.
\begin{itemize}
\item All regular monomorphisms and is closed under composition and pullback. 
\item The unique morphism $!\of\cod(\gen(M))\to 1$ is also a member of $S$.
\end{itemize}

A functor $\code{M}:\Core(\Func([1],\cat S))\to \cat S$ represents the class of $M$ morphisms inside of the internal category $\cat S$. That functor sends each morphism $f$ to the object of pullback squares $f\to\igen(M)$. Here $\igen(M)$ is a global section of $\Ar(\cat S)$ which is the characteristic morphism of the commutative triangle that contains the $S$ morphisms $\gen(M)$ and $!\of\cod(\gen(M))\to 1$. 
\[\xymatrix{
\bullet \ar[rr]^{\gen(M)}\ar[d] &&\bullet \ar[rr]\ar[d] && 1 \ar[d]^{\igen(M)}\\
\Sigma f\of \Ar(\cat S).\dom(f)\ar[rr]_{(x,f)\mapsto (f(x),f)} && \Sigma f\of \Ar(\cat S).\cod(f)\ar[rr]_(.6){(y,f)\mapsto f} && \Ar(\cat S)
}\]

An $M$ morphism between simplicial objects is a morphism $f\of X\to Y$ such that for each $i\of\nno$ each $f[i]$ is a member of $M$.
From $\code M:\Core(\Func([1],\cat S))\to \cat S$ comes a functor $\code{M\s}:\Core(\Func([1],\cat S\s))\to \Func(\nno\disc,\cat S)$ that represents this class of morphisms.

\subsection{Lifted generic}%here... what is $\igen(M/s)$ ?
Functors that represent classes have their own notion of generic morphism. Every morphism $g$ of an internal Heyting bialgebra $\cat C$ induces a object-of-pullbacks-functor $P\of\Core(\Func([1],\cat C))\to \cat C$. The morphism $g$ is generic for some other functor $G\of\Core(\Func([1],\cat C))\to \cat C$ is there is a natural transformation $G\to P$. For $\code M$ this morphism is identity by definition. This section show that how lift the generic morphism $\igen(M)$ of $\code M$ in $\cat S$ to a generic morphism $\igen(M\s)$ of $\code{M\s}$.

The way to get there is a bit roundabout. For each simplicial object $V$ there is an isomorphism between $V[n]$ and $\simplex[n]\to V$ by the Yoneda lemma. If $g\of U\to V$ is a generic morphism, then there must a natural equivalence between members of $V[n]$ and morphisms $\bullet\to \simplex[n]$ that belong to the class. By lemma \ref{slice equivalence} morphisms $\bullet\to \simplex[n]$ correspond to functors $\simCat/[n]\dual\to\cat S$ whose values are fibres of the generic morphism $\igen(M)$ of $M$. These in turn are members of $\simCat/[n]\dual\to\cof(\gen(M))^{\simCat/[n]\dual}$.

\begin{lemma}
For each $n\of\nno$ let $\igen(M\s)[n]$ be the pullback of $\igen(M)$ along the map $F\mapsto F(\id_{[n]})\of\cof(\gen(M))^{\simCat/[n]\dual}\to \cod(\gen(M))$. The morphism $\igen(M\s)$ is a generic morphism of $M\s$.
\end{lemma}
...
This subsection shows that $\gen(M\s\f)$ has the following property.
...
\hide{ About the Bezem-Coquand counterexample: it shows something about the homotopy equivalence of the fibres, while I only proof the weak equivalence of the fibres. When these equivalences coincide (cofibrant codomains) there is no undecidable degeneracy. My best guess is that the codomain in the counterexample is not cofibrant, at least not in the sense defined in the paper.}


\section{Realizability categories}%here
%hoe ver gaan?
