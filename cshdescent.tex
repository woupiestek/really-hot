...
\section{Descent}
This purpose of this section is to replace arguments based on \emph{minimal fibrations} in simplicial homotopy theory, in particular those that are related to homotopy type theory. Assume that there is a class $M$ of \emph{modest fibrations} in $\ambient\s$ with the following properties.
\begin{itemize}
...
This requirement may give us more limits than we need, but the intended model of modest morphisms in the ex/lex completion of the category of assemblies satisfies it.
...
Since $\ambient$ may lack a subobject classifier, there can be irregular monomorphisms.
\end{itemize} 
...
There is a $\chi(D(v\ri(\uni)))\of\simplex[n]\to V$ and $\chi(h_*(v\ri(\uni)))\circ h$ equals $v$ by definition \ref{universal modest fibration}. These fillers forms a filler operator, which make $\bang\of V\to 1$ a fibration. %is there no more to understand here?
\end{proof}
...
\begin{definition}[Face notation] We introduce a notation for monomorphisms in $\simCat/[n]$ or more accurately for the result of applying the functor $\simplex\of \simCat/[n] \to \ambient\s/\simplex[n]$ to them. For $\xi\of[m]\to[n]$ and $\phi\of[m]\to\bool$ let $\face(\xi,p)\of \dom(\face(\xi,p))\to \simplex[m]$ satisfy
\begin{align*}
