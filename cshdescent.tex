...
If $\ambient$ has infinite colimits, the functor $D$ has a left adjoint $K'$ that satisfies $K'\simplex\simeq K$. We cannot rely on that property here. The functor $K$ does to finite colimits or simplices, including horn inclusions like $\horn_l[m]\to\simplex[m]$. The reason $D$ preserves fibrations and their right lifting property, is that its partial left adjoint preserves the left lifting property. The following proof shows this that the result of applying $K$ to a horn inclusion is the pushout of a composition of sums of horn inclusions.
...
u_i(x) &= (K_0\xi(x)\neq i)\\
v_{i\of[m]} &\of[m+\norm\xi]\to 2\\
v_i(\kappa(\xi)(\vec x)) &= \bigvee_{j\neq k} (x_j\neq i)\\
v_i(\lambda(\xi)(x)) &= (x\neq i)
\end{align*}


\hide{The fibration $Df$ is the result of gluing new simplices to $h\circ f$ in order to provide fillers for horn inclusion that don't factor through $f$. The right adjoint $h_*$ of the pullback functor $h\ri$, which satisfies $h_*(f) = (h\circ f)^h_{\simplex[n]}$, glues many new simplices to $h\circ f$. Unfortunately, it glues in so many new simplices, that $h_*(f)$ is not weakly equivalent to $f$ anymore.

The functor $K$ sends each object $\xi\of[m]\to[n]$ of $\simCat/[n]$ to a homotopy between the pullback of $\simplex(\xi)\of\simplex[m]\to\simplex[n]$ and a simplex in the fibre over the vertex $k$ of $\horn_k[n]$. The functor $D$ is a modification of $h_*$ that only glues in new simplices when these homotopies exist. That guarantees weak equivalence and preservation of fibration.}
