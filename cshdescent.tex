...
\begin{theorem} If $\uni\of U\to V$ is a universal modest fibration, then $V$ is fibrant. \end{theorem}
...
The relation of $D$ and $K$ allows us to reduce the problem of lifting a horn against $Df$ to the problem of lifting some finite colimit of objects in the image of $K$ against $f$. These finite colimits have the required left lifting property by lemma %...
\end{proof}
...
The lexical product means that $\Pi i\of([n]-\set k).\xi_i$ get the lexical ordering. The map $K_0$ adds the lexicographical product of most of the fibres of a map $\xi\of[m]\to[n]$ to the fibre over $k$. %more?
...
\item Let $K_1(\phi)\of K_0(\xi)\to K_0(\xi')$ be the unique nondecreasing function which satisfies $K_1(\phi)\circ \ka(\xi) = \ka(\xi')\circ \Pi i\of([n]-\set k).\phi_i$ and $K_0(\phi)\circ \la(\xi) = \la(\xi')\circ \phi$.
\end{enumerate}
...
Thanks to the following property, the natural equivalence of homset above extends to horn inclusions.
...
\begin{align*}
\base(\dom(\face(\xi,p))) &= \set{\alpha\of\base(\simplex[m])\middle|\forall i\of \dom(\alpha).\phi\circ\alpha(i)=1}\\
\dim(\alpha) &= \dom(\alpha)\\
\alpha\cdot\chi &= \alpha\cdot\chi\\
\face(\xi,p)(\alpha) &= \alpha
\end{align*}
\[\xymatrix{
\bullet\ar[rr]^{\face(\xi,\phi)}\ar[dr] && \simplex[m]\ar[dl]^{\simplex(\xi)} \\
& \simplex[n]
}\]
\end{definition}
...
\begin{align}
&\horn_l[m]\stackrel{\hat h}\to \simplex[m]\stackrel{\simplex(\xi)}\to \simplex[n] = \bigvee_{i\of [m]-\set l} \face(\xi,x\mapsto x\neq i)\\
...
\begin{align*}
u_i(x) &= (K_0\xi(x)\neq i) & 
v_i(\kappa(\xi)(\vec x)) &= \bigwedge_{j\of [n]-\set k} (x_j\neq i)\\
v_i(\lambda(\xi)(x)) &= (x\neq i)&
...
has the left lifting property with respect to all fibrations.\end{lemma}
...
Each inclusion $A_j\to A_{j+1}$ is a pushout of the map $A_j\cap \face(K_0(\xi),u_{j+1}) \to \face(K_0(\xi),u_{j+1})$, where
...
The faces $\face(K_0(\xi),u_{j+1}\land v_{i})$ and $\face(K_0(\xi),u_i\land u_{j+1})$ all contain the point $\lambda(l)$ of $\simplex[m+\norm\xi]$ which means that lemma \ref{face completion} applies to the inclusion $A_j\cap \face(K_0(\xi),u_{j+1}) \to \face(K_0(\xi),u_{j+1})$. Hence $A\to B$ is a cofibration.

If $\xi(l)=k$, then $B=C$ and the proof is complete. So suppose $\xi(l)=i\neq k$. First define the following maps $[m+\norm\xi]\to\bool$ for each $\vec p \of\prod_{i\of [n]-\set k}\xi_i$.
\begin{align*}
...
Each inclusion $B_j\to B_{j+1}$ is a pushout of the map $B_j\cap \face(K_0(\xi),u_{\xi(l)}\land w_{\vec p}) \to \face(K_0(\xi),u_{\xi(l)}\land w_{\vec p})$, where
\begin{align*} B_{\kappa(\vec p)}\cap \face(K_0(\xi),\land w_{\vec p}) = 
&\bigvee_{i\of [n]-\set{\xi(l),k}}\face(K_0\xi,u_i\land w_{\vec p})\vee\\
&\bigvee_{i\of [m]-\set l} \face(K_0\xi,v_i\land w_{\vec p})\vee\\
&\bigvee_{\kappa(\vec q)<\kappa(\vec p)} \face(K_0\xi,w_{\vec q}\land w_{\vec p})
\end{align*}
Let $\vec p[l]\of \prod_{i\of [n]-\set k}\xi_i$ satisfy $p[l]_j = l$ if $\xi(l)=j$ and $p[l]_i=p_i$ otherwise. The point $\kappa(p[l])$ is a member of $\face(K_0\xi,u_i\land w_{\vec p})$ for all $i\of [n]-\set{\xi(l),k}$. It is also a member of $\face(K_0\xi,w_{\vec q}\land w_{\vec p})$ for all $\vec q\of \prod_{i\of [n]-\set k}\xi_i$ such that $\kappa(\vec q)<\kappa(\vec p)$. In the remaining faces $\face(K_0\xi,v_i\land w_{\vec p})$ for $i\of [m]-\set l$, if not $v_i(\kappa(\vec p[l]))$ then by definition not $\bigwedge_{j\of [n]-\set k} (p[l]_j\neq i)$. So there is a $j\of [n]-\set k$ such that $p[l]_j=i$.
\hide{the hardest part has yet to come} %here
\end{proof}
...
\hide{ We can break this up into faces already\dots }

\hide{turn this into a bunch of lemma's:

The following proof shows this that the result of applying $K$ to a horn inclusion is the pushout of a composition of sums of horn inclusions.

It is all pointless. I am stuck repeating the same explanation over an over again. It does not get any better.

There is no compromise between exactness and clarity here.
}
