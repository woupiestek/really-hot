...
When $j\neq\kappa(\vec p)$ for any $\vec p \of \prod_{i\of [n]-\set k}\xi_i$, the inclusion $B_j\to B_{j+1}$ is the identity arrow, which satsifies the left lifting property trivially. 

Suppose $j\neq\kappa(\vec p)$. Each inclusion $B_j\to B_{j+1}$ is a pushout of the map $B_j\cap \face(K_0(\xi),u_{\xi(l)}\land w_{\vec p}) \to \face(K_0(\xi),u_{\xi(l)}\land w_{\vec p})$, where
...
We conclude with a simple but fundamental lemma about monomorphisms that have the left lifting property. 
