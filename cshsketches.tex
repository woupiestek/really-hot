...
Since pushout preserve left lifting properties, a filler operator for the pushout product is sufficient.
...
%here
The case where $f$ is the family of horn inclusions is more complicated.
For each $m,n\of\nno$ the faces of $\simplex[m]\times\simplex[n]$ coinside with morphisms $f\of[m+n]\to[m]\times[n]$ that satisfy $f(p)_0+f_1(p)=p$. 
\end{proof}
