...
\begin{proof} 
The object of lifting problems $P$ is $\hom_{\set\geq}(\horn_{\set\geq},V)$. There is an indexed coproduct of horn inclusions $k_P\of \horn_P\to\simplex_P$ and the fibrancy of $V$ is equivalent to factoring the canonical morphism $v\of\horn_P\to V$ through it.\hide{Explain how this is constructed?}
The modest fibration $v\ri(\uni)\of \horn_P\times_V U \to \horn_P$ descends along $h$ to form a modest fibration $D(v\ri(\uni))\of \bullet\to\simplex_P$ by lemma \ref{descent}.
Hence there is a $\chi(D(v\ri(\uni)))\of\simplex_P\to V$ and $\chi(D(v\ri(\uni)))\circ k_P$ equals $v$ by definition \ref{universal modest fibration}.
\[\xymatrix{
\horn_P\times_V U \ar@/^2ex/[rrr] \ar[r]\ar[d]_{v\ri(\uni)} & \bullet \ar[rr]\ar[d]_{D(v\ri(\uni))} && U\ar[d]^\uni\\
\horn_P\ar@/_2ex/[rrr]_v\ar[r]^{k_P} & \simplex_P\ar[rr]^{\chi(D(v\ri(\uni)))} && V
}\]
\end{proof}

\begin{lemma}
For each fibration $f\of X\to \horn_P$ there is a fibration $Df\of Y\to\simplex_P$ such that $f$ is the pullback of $Df$ along $h$. Moreover, if $f$ is modest, so is $Df$. \hide{is this last bit proven anywhere?} \label{descent}
\end{lemma}
...
\mathrmdef{supp}
\begin{proof}
Each slice $\ambient\s/X$ of $\ambient\s$ is isomorphic to the category of internal presheaves over the internal \emph{category of elements} of $\elts X$ in $\ambient\s$--see definition \ref{category of elements} and lemma \ref{slices are presheaves}. %definition %lemma
Internal functors induce reindexing functors, that have both a left and a right adjoint--see lemma \ref{both adjoints}. %lemma
The functor $D = K^*\circ (k_P)_*$ where $K$ is a specific internal endofunctor of $\elts(\simplex_P)$, and $(k_P)_*$ is the direct image along $k_P\of \horn_P\to\simplex_P$. Both reindexing and direct images preserve modest morphisms, so $Df$ is modest if $f$ is. %lemma?
The functor $D$ has a left adjoint $h\ri\circ K^*$ which can be shown to preserve discrete families of cofibrations. %lemma
For that reason, $D$ preserves all fibrations, including all modest fibrations.
\end{proof}

%Let's skip definitions that can be found in M&M

\begin{definition} The \keyword{category of elements} $\elts(X)$ of a simplicial object $X$ is the internal category of $\ambient$ whose objects are the member of $\base X$ and where a morphism $x\to y$ is a morphism of $\simCat$ between $[\dim(x)]\to[\dim(y)]$. Identities and composition of these morphism is the same as in $\simCat$.
\end{definition}

\begin{lemma}
The category $\ambient\s/X$ is isomorphic to the category of internal presheaves $\psh(\elts(X))$. \label{slices are presheaves}
\end{lemma}

\begin{proof} The categories are the same modulo some choices of encoding. \end{proof}

\begin{definition} Let $F\of \cat C\to\cat D$ be an internal functor. 
The \emph{reindexing functor} $F\ri$ sends $X=\tuplet{\base X,\supp,\cdot}$ of $\psh(\cat D)$ to $X'=\tuplet{\base(X'),\supp,\cdot}$ of $\psh(\cat C)$, where:
\begin{align*}
X' &= \set{\tuplet{x,c}|F_0(c) = p(x)}\\
\supp &= F_0\ri(\supp)\\
\tuplet{x,c}\cdot f &= \tuplet{x\cdot F_1(f),F_0(\dom(f))}
\end{align*}
Here $F_0$ is the mapping of objects, and $F_1$ the mapping of morphisms.
A morphism $f\of \tuplet{X,p,\cdot}\to \tuplet{Y,q,\cdot}$ is a morphism $f\of X\to Y$ and $F\ri(f) = F_0\ri(f)$. Preservation of composition and identities is automatic.
\end{definition}

\begin{lemma}
For internal functors in $\ambient$ the reindexing functors have both adjoints.\label{both adjoints}
\end{lemma}


\begin{proof}
The basic intuition is that categories of presheaves have all limited and colimits that are indexed over the category $\ambient$. The ordinary constructions of the left and right adjoints of the reindexing functors there work here as well as anywhere else. This idea is worked out below for the skeptics.

That they have a right adjoint following form the fact that $\ambient$ is locally Cartesian closed.

Let $F\of\cat C\to\cat D$ be an internal functor and let $X=\tuplet{\base X,\supp,\cdot}$ be an object of $\psh(\cat C)$. Define $F_*(X)$ as follows.
\begin{align*}
\base(F_*(X)) &= \set{ \tuplet{d,\phi} | d\of D_1, \phi\of (F/d) \to \base(X), p\circ \phi = \pi_0, \phi(f\circ F_1(g)) = \phi(f)\cdot F_1(g) }\\
\textrm{where }&(F/d) = \set{\tuplet{c,f}| c\of C_0, f\of F_1(c)\to d}\\
\textrm{and }&\pi_0\tuplet{c,f} = c\\
\supp\tuplet{c,d,\phi} &= d \\
\tuplet{c,d,\phi}\cdot\chi &= \tuplet{d,\dom(\chi),\function f\phi(f)\cdot\chi}
\end{align*}
For morphisms $f\of X\to Y$ let $F_*(f)(\tuplet{d,\phi}) = \tuplet{d,f\circ \phi}$.

For $f\of X\to F_*(Y)$, the transpose $f^t\of F^*(X) \to Y$ satisfies $f^t\tuplet{x,\xi} = f(x)\tuplet{\cod(\xi),\xi}$. Vice versa for $g\of F^*(X) \to Y$ the transpose $g^t\of X\to F_*(Y)$ satisfies $g^t(x)\tuplet{d,\xi} = g\tuplet{x,\xi}$, and these transpositions are clearly each other inverses, proving $F_*$ right adjoint to $F\ri$

For the left adjoint, exactness and the $\nno$ provide all coequalizers.
\begin{align*}
\base(F_!(X)) &= \set{ \tuplet{x,\phi} | \dom(\phi) = F_0(p(x)) } / E \\
\textrm{where }& E = \set{\tuplet{\tuplet{c\cdot\phi,\chi},\tuplet{c,F_1(\phi)\circ\chi}}| x\of\base X, \phi\of c\to p(x), \chi\of d\to F_0(c)}\\
\supp\tuplet{x,\phi} &= \dom(\phi)\\
\tuplet{x,\phi}\cdot\chi &= \tuplet{x,\phi\circ\chi}\\
\end{align*}
For a morphism $f\of X\to Y$ let $F_!(f)\tuplet{x,\phi} = \tuplet{f(x),\phi}$.

To prove that this quotient is a left adjoint, consider that $\psh(\cat D)$ is a category of algebras for a monad on $\ambient/D_0$. Among the algebras that admit an embedding of $F_0\circ \supp$, and left the action of $\cat C$ to that of $\cat D$, the algebra given by $F_1(X)$ is merely the initial one. That already determines that we have an adjunction.
\end{proof}

\hide{It would be worth finding a source on internal categories and internal presheaves, that has this all worked out meticulously already}




\begin{definition}
Let $P$ be the following simplicial object:
\begin{align*}
\base P &= \set{\tuplet{\xi,k}|\xi\of\Ar(\simCat),k\of[\cod\xi]}\\
\dim\tuplet{\xi,k} &= \dom(\xi) \\
\tuplet{\xi,k}\cdot\phi &= \tuplet{\xi\circ\phi,k}
\end{align*}

Let $K\of\elts(P)\to\elts(P)$ be the following functor:
\begin{align*}
K_0\tuplet{\xi,k} &= \tuplet{\xi',k} \\
\textrm{where } \dom(\xi') &= \cod(\xi) + \prod_{i\of [\cod(\xi)]-\set k}\#\xi_i\\
\textrm{where } \prod_{i\of [\cod(\xi)]-\set k}\#\xi_i
\textrm{where } \xi_i &= \set{j\of \dom(\xi)|\xi(j)=i}\\
\textrm{and } \#\emptyset &= 0\\
\textrm{and } \# x &= \max(x) - \min(x) + 1\\
\\
\xi'(i) &= \xi(i) \textrm{ if } \xi(i) < j\\
\xi'(i) &= \xi(i-) \textrm{ if } \xi(i) > j
\end{align*}
%perhaps just describe the operators as functors of \simCat?
...
\section{Arithmetic in $\simCat$}
\begin{definition}

The \emph{lexicographical product} is a functor $LP\of\simCat/\simCat\to \simCat$. It takes $\phi\of[n]\to [m]$ to 


For $m$ and $n\of \nno$ the \emph{lexicographical product} of $[m]$ and $[n]$ is 
