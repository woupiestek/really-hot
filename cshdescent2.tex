...
\begin{lemma}
For each fibration $f\of X\to \horn_P$ there is a fibration $D(f)\of D(X)\to\simplex_P$ such that $f$ is the pullback of $Df$ along $k_P$. Moreover, if $f$ is in $M$, so is $Df$. \hide{is this last bit proven anywhere?} \label{descent}
\end{lemma}
...
such that:
%these aren't the conditions we actually need.
\begin{itemize}
\item The pullback $s_\xi\ri(k_P)\of \horn_P\times_{\simplex_P}S_\xi \to S_\xi$ is an acyclic cofibration. %lemma
\item If $\xi\of\base(\horn_P)\subseteq \base(\simplex_P)$, then $s_\xi$ is isomorphic to the $\xi$ face of $\horn_P$. %lemma
\end{itemize}
The faces of the underlying object of $D(X)$ are maps $s_\xi\to(k_P)_*(f)$--or equivalently maps $k_P\ri(s_\xi)\to f$. %def
These guarantee that $D(f)$ is a fibration and that $k_P\ri(Df) \simeq f$. %lemma
Moreover, because the underlying map of $Df$ is a dependent product of a member of $M$, the underlying map of $Df$ belongs to $M$ as well. %lemma
\[\xymatrix{
...
Where $d_i$ is the \keyword{face map} $[\cod(\xi)-1]\to[\cod(\xi)]$ that satisfies $d_i(j) = j$ if $j<i$ and $d_i(j) = j+1$ if $j\geq i$.
\begin{align*}
...
Partially ordered objects like $(X,\leq)$ have a \keyword{nerve} $N(X,\leq)$ which is a simplicial object. The object $\base(N(X,\leq))$ consist of \emph{non decreasing} maps $[n]\to X$ for arbitrary $n\of\nno$. 
