...
\item for each monomorphism $m$ of $\cat P$, $\foh_\LP\ri(S(m))$ is an acyclic cofibration.
\end{itemize}
...
\[\xymatrix{
N(S,\leq)\ar[d]_s\ar[r]^t\ar[dr]^p & \simplex_\LP \ar[d] \\
\simplex_\LP\ar[r]&\LP
}\]
\end{definition}

The functor from the proof of \ref{descent} $\cat P\to \ambient^{\cat P\dual}$ is encapsulated in the structure defined above, which has the structure of a fibred category over the pullback of $s$ and $t$, for a suitable notion of morphism.

\begin{definition}
\hide{There is an equivalence relation $\sim$ on $S$ generated by the pairs:
\begin{align*} 
\tuplet{p,\sigma\circ\phi,0,j} &\sim \tuplet{p,\sigma,0,\phi(j)}\\
\tuplet{p,\sigma\circ\phi,1,j} &\sim \tuplet{p,\sigma,1,\phi\circ j}
\end{align*}
This extends to an equivalence relation $N(S,\leq)$ also denoted with $\sim$.}

%here

\[ \product{z}\set{x\of N(S,\leq)|t(x) = z}\to
\set{\tuplet{x,y}\of X\times N(S,\leq)| f(x) = s(y),t(y)=z } \]
