...
The \emph{reindexing functor} $F\ri$ sends $\tuplet{X,p,\cdot}$ of $\psh(\cat D)$ to $\tuplet{X',p',\cdot}$ of $\psh(\cat C)$, where:
\begin{align*}
...
\hide{
Keep thinking $F_*(P)(X) \simeq \nat(F\ri(\hom(_,X)),P)$, and that would fit the left adjoints story wouldn't it?
}
%here
For the left adjoint, all part of the structure play a role. We use the fact that $\ambient$ 
\end{proof}
