...
\end{align*}
%doubts: it is hard to tell whether the result is even still monotone.

There is a left action of $\simCat$ on $S$ as well--fortunately simpler. 
\begin{align*}
\codim(\tuplet{\xi,k,f,g}) &= \dom(\xi)\\
\phi\cdot\tuplet{\xi\circ \phi,k,f,g} &= \tuplet{\xi,k,\phi\circ f,
\phi\circ g}
\end{align*} %here, not actually an action from simcat!

Define the morphism $s\of S\to \simplex_{\leq}$ as follows.
\begin{align*}
