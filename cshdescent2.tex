...
\begin{proof} The set of lifting problems $P$ to solve is isomorphic to $\hom_{\set\geq}(\horn_{\set\geq},V)$. There is an indexed coproduct of horn inclusions $k_P\of \horn_P\to\simplex_P$ and the fibrancy of $V$ is equivalent to factoring the canonical morphism $v\of\horn_P\to V$ through it.\hide{Explain how this is constructed?}
...
\begin{proof} 
There is no horn inclusion for $n=0$. In the case $n=1$, there are two maps $1\to\simplex[1]$. In both cases $Df=\pi_0\of \simplex[1]\times X \to\simplex[1]$ suffices. For all the cases were $n>0$ we use the following construction.
...
\[ \ambient\s/\simplex_P(\simplex(\xi), D(f))\simeq \ambient\s/\horn_P(K(\xi),f) \]


This is possible because $\ambient\s/\simplex_P$ is equivalent to the category of presheaves over \dots and the Yoneda lemma applies to those presheaves. %lemma?
