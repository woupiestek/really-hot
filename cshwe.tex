...
\begin{lemma} Let $f\of X\to Y$ and $g\of Y\to Z$ be morphisms of $\ambient\s\f$. If any two of $f,g$ or $g\circ f$ are weak equivalences, then all three are. \label{toot}\end{lemma}
...
By proposition \ref{factor1} $g\circ f=h\circ k$ for some acyclic fibration $h\of W\to Z$ and a cofibration $k\of X\to W$. The rest of this proof shows that $k$ is weakly invertible (see definition \ref{weakly invertible}) if $g$ is. Since acyclic cofibrations are weakly invertible and vice versa--as proved in lemmas \ref{acyclic have lifting} and \ref{lifting is acyclic}--$g\circ f$ is a composition of an acyclic fibration following an acyclic cofibration and therefore a weak equivalence.
...
\begin{proof} Being acyclic is equivalent to being contractible (see definition \ref{contractible}) by lemma \ref{acyclic means contractible} and lemma \ref{Reedy}. Reasoning dual to that in the proof of lemma \ref{left cancellation} gives $g$ the right lifting property for the family of cycle inclusions.

\[\xy
(14,28)*+{X}="x", (0,21)*+{\cycle_\nno}="v", (14,14)*+{Y}="y", (0,7)*+{\simplex_\nno}="w", (14,0)*+{Z}="z"
\ar@{.>}^(.67){a'} "v";"x" \ar_(.67)a "v";"y" \ar "v";"w"
\ar@{.>}_(.75){b'} "w";"x" \ar_(.67)b "w";"z"
\ar^f "x";"y" \ar^g "y";"z"
\endxy\]

Dual reasoning works because the unique morphism $\bang\of 0\to\cycle_\nno$ is a cofibration. A member of $\base(\cycle_\nno)$ is a face if it is a monomorphism of $\simCat$ and this is a decidable property. No faces are in the image of $\bang$.
\end{proof}
