...
%this needs work
If the ambient category $\cat A$ is locally Cartesian closed and has stable coequalizers, then the density comonad can be internalized as well, at the expense of the universal property.

\begin{definition}
The \emph{density comonad} for a diagram $\tuplet{f,\cdot}$ sends $A$ of $\cat A$ to the the coend $\left(\int^{c\of \icat C}A^{f_c}\times f_c\right)$--where $f_c = \set{x\of X|f(x)=c}$. This is a coequalizer of the parallel pair $p,q\of A\to B$ between pullbacks show below, instead of an infinite coproduct.
\begin{align*}
p&=\lambda\tuplet{x,y,z}.\tuplet{y(x \cdot -),z}\\
q&=\lambda\tuplet{x,y,z}.\tuplet{y,x\cdot z}\\
\textrm{where }A&= 
\set{ \tuplet{x\of C_1, y\of A^{f}, z\of X }| \cod(x)=\dom(y), \dom(x) = f(z)}\\
\textrm{and }B&=
\set{ \tuplet{x\of A^f, y\of X} | \dom(x) = f(y) }
\end{align*}
\end{definition}
\hide{
Notation for locally Cartesian closed categories: 
\[ A^f &= \sum_{x\of\cod(f)}A^{\set{y\of \dom(f)|f(y)=x}}\]
}
...
% closed under composition... presumably
% generic

%idea: keep it down to necessities,
%introdce the constructions later
