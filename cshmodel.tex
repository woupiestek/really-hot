...
A $y\of\base Y$ is \keyword{degenerate} if $y=y'\cdot\epsilon$ for an arbitrary $y'\of\base Y$ and an epic $\epsilon\of\Ar(\simCat)$ that is not an identity morphism. A \keyword{face} of $Y$ is a $y\of\base Y$ which is \keyword{nondegenerate}. Hence $f$ is a cofibration if there is a morphism $\psi\of\base Y\to\bool$ such that $\psi(y)=\false$ if an only if $y$ is either degenerate or in the image of $f$.\end{definition}
