...
\begin{definition} An \keyword{model structure} on an $\cat A$-enriched category $\cat C$ consist of three sets of morphism: the \emph{fibrations} $F$, the \emph{cofibrations} $C$ and the \emph{weak equivalences} $W$ which have the following properties. 
\begin{itemize}
...
\begin{example} In the category of simplicial sets all monomorphism are cofibrations. Decidability comes for free there.\end{example}
